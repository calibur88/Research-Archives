...
3. **实验签章**：维也纳大学 2026 年 Nature 发表的 MUSCLE 纳米团簇干涉实验（Pedalino et al., *Nature* **649**, 866–870）的可见度–激光功率曲线与 NBG 框架的序参量 $C(\mu)$ 演化在定性上完全对齐，且框架预言了该实验在 log-binning 分析下应观测到的阶跃峰位跳变。
...
本文在 NBG 类论框架内建立了一套严格的公理体系，证明量子非对易性、连续谱、波粒二象性、迹反常乃至"隧穿"现象，均可归结为**递归柯西刚性在某一序数层级上的破缺**，以及**逆梅林变换投影的几何畸变**。我们进一步通过数值实验和与最新干涉实验的对接，展示了该框架的可计算性与可证伪性。
...
标准 QM 将 MUSCLE 实验的可见度曲线（图 2b：可见度 $V$ 随 G2 光栅激光功率 $P_2$ 先升后降）解释为量子退相干与经典阴影的竞争。NBG 框架提供了新的映射：
...
| 量子模型 vs 经典模型 | 预临界刚性区 vs 过临界溢出区 |

在该映射下：
- **低功率区**（$P_2$ 小，$\lambda$ 小）：可见度缓慢上升 $\leftrightarrow$ 聚焦度缓慢爬升，系统处于预临界刚性区；
- **临界阈值**（$P_2 = P_c$，$\lambda = \lambda_c$）：可见度达到峰值后断崖式下跌 $\leftrightarrow$ $C(\mu)$ 跌落、Lyapunov 指数暴涨，拓扑溢出发生；
- **高功率饱和区**（$P_2 > P_c$，$\lambda > \lambda_c$）：可见度稳定低值 $\leftrightarrow$ 绿色填充区拓扑溢出稳态，系统进入过临界分支。

### 13.3 可检验预言

NBG 框架对 MUSCLE 实验提出了**两个可检验的定量预言**：

1. **Log-binning 峰位跳变**：若对原始计数数据（Zenodo: 10.5281/zenodo.17502163）按对数尺度（$\ln x$）进行空间 binning，则在临界激光功率 $P_c$ 附近应观测到计数峰位的**阶跃式跳变**（从 $x_1$ 跳到 $x_2$），而非标准 QM 预言的连续高斯展宽。标准 QM 的线性位移扫描会平滑掉这一阶跃，但 log-binning 应能恢复。

2. **质量分辨临界功率**：团簇质量分布具有一定宽度（$\Delta m/m = 0.32$）。NBG 框架预言：不同质量子群（对应不同谱密度 $\rho$）的临界功率 $P_c$ 应不同——质量越大（$\rho$ 越高），极点压缩越强，临界阈值越低。这表现为可见度曲线的**多阶梯跌落**，而非单阶梯过渡。

### 13.4 实验意义

MUSCLE 实验是首个进入 NBG 框架预言的"高密度-强耦合"参数区的实验。标准 QM 在该区域仅提供"退相干"的模糊解释，而 NBG 框架给出了**离散的相变机制**（拓扑荷整数溢出）和**精确的序参量演化**（$C(\mu)$ 曲线）。若上述两个预言被验证，将为乘法基底物理提供直接的实验签章。
...
**关键机制**：当通过该围道积分将梅林空间的乘法算子（谱参数 $s$）投射到傅里叶空间的加法算子（动量 $p$）时，复 $s$ 平面上的割线跨越强制引入相位因子 $e^{i\pi s/2}$。该因子的留数贡献使原本在梅林空间纯乘法的 $s$ 与 $p$ 发生虚数单位耦合。

具体地，梅林空间的复合算子差 $xp - px = i$（常数）在傅里叶投影后被重新诠释为加法算子的非对易子：
...
- **2026-07-16**：重大修订。
  1. 增补**不可通约性定理**（§11），严格证明乘法基底与加法基底在观测上的不可通约性；
  2. 增补**几何相变与高密度团簇"隧穿"重释**（§12），提出拓扑荷整数溢出（integer overflow）机制，预言阶跃式峰位跳变；
  3. 增补**实验对接章节**（§13），与维也纳大学 MUSCLE 纳米团簇干涉实验（Pedalino et al., *Nature* 2026）进行定量映射，提出 log-binning 峰位跳变与质量分辨临界功率两个可检验预言；
  4. 全面更新附录 B，纳入七组数值签章（Groups I–VII），涵盖谱统计、干涉相变、闭环验证、基底投影、收敛几何、全模式演化与几何相变；
  5. 更新摘要、引言与结论，反映最新理论进展。
