...
**性质 7.3（三三成 120°）**  
对任意 $k$，三元组 $(w_k, w_{k+1}, w_{k+2})$ 在复平面上的相对相位严格为 $120^\circ$（由 $r$ 的单位根性质保证），满足公理 Ⅱ 的局部对称性。
...
**证明**：$\eta$ 的调整不改变 $r^k$ 的 $120^\circ$ 循环结构（公理 Ⅱ 自动满足），仅改变绝对平移位置，因此 $\mathrm{Ob}_{12}^{(\eta)} = 0$ 对所有 $\eta \in \mathfrak{R}_{12}$ 成立。
...
| **DCG 3.0** | 新增第七部分「无穷维度的统一构造」，采用等比等差双模递推 $w_k = r^k + k \cdot d$ 给出任意 $n \geq 3$ 轴情形的完整代数构造；解陪域显示形式由等差参数 $d$ 自然给出；无穷维度从"展望"升格为"已完成构造"；版本定稿。 |