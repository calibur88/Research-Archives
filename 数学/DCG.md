...
| **DCG 3.2** | 新增「可选公理 Ⅵ & Ⅶ」独立章节。可选公理 Ⅵ（唯一性）：引入自生递归序列定义，要求生成内禀性与参数闭合性（Kuratowski 集合极限），规定闭包须为内生生成规则下唯一确定的极限几何体；可选公理 Ⅶ（复合性）：引入复合递归生成定义，要求组件合法性、自证完备性与非平凡性，同时声明 Ⅵ 时追加交叉约束（复合分解唯一性）；新增可选公理总则与声明组合检验范围表，明确声明但核验失败时仅撤销可选标签、不剥夺公理 Ⅰ–Ⅴ 基础合法地位。 |