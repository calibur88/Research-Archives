...
DCG 并非唯一几何学，而是特定合法性框架。

本体系所列公理 Ⅰ–Ⅴ 为选择性约束。任何构造者有权拒绝其中任意一条乃至全部，并在无约束条件下自由操作。然而，**合法性**是 DCG 框架内的专属判定词；凡未接受并满足上述公理者，DCG 不对其授予「几何闭包」之资格，亦不评价其「合法」或「非法」——彼等构造仅处于 DCG 的未定义域中，与本体系无涉。

DCG 不对欧氏几何、闵氏几何或其他公理体系内的构造作合法性评判。各体系相互独立，仅在自愿接受 DCG 公理约束时，DCG 方启动其合法性检验程序。DCG 无意替代任何既有几何学，亦不宣称自身为唯一真体系。

**体系定位**：DCG 是一个元几何框架，它以五条操作性公理定义「合法几何闭包」的判定准则。凡满足这五条公理的构造——无论欧氏、分形、微分几何、$q$-变形空间或高阶表示层——均在 DCG 中被承认为合法几何对象。凡不满足的构造，DCG 不评价，仅将其置于未定义域中。

**操作顺序原则**：在 DCG 中，**构造优先于测量**。闭包必须首先通过公理 Ⅰ–Ⅳ 的检验成为合法几何对象，方可对其进行积分（公理 Ⅱ、Ⅴ 中出现的积分运算均在此原则下进行）。积分不参与构造，而是构造完成后的外边界性质提取。

**并置操作的积分规则**：
- 对于两个子域 $\mathcal{D}_1, \mathcal{D}_2$，其并置闭包 $\mathcal{D} = \mathcal{D}_1 \cup \mathcal{D}_2$ 的积分 $\mathcal{Z}_q(\mathcal{D})$ **仅在以下情形**等于 $\mathcal{Z}_q(\mathcal{D}_1) + \mathcal{Z}_q(\mathcal{D}_2)$：
...
- 空间域可以是无穷维的——当域变量的数目为无穷时（例如边界由任意函数描述）。这种"无穷"仅代表**切法的选择无穷**，绝不代表底流形 $\mathbb{R}^3$ 的拓扑维数增加。
- DCG 中凡出现"无穷"，在没有额外限定时，默认指**域变量维数的无穷**，而非几何拓扑维数。
...
- 无穷维的来源是**映射的像值自由度**——函数在无穷多个点上的取值各自独立变化。
- 底空间固定不变，拓扑维数不增加。
...
| 特征 | 标准泛函分析 | DCG 空间域 |
|:---|:---|:---|
| 空间中的"点" | 函数 $f: X \to \mathbb{R}$ | 几何点 $P \in \mathcal{D} \subset \mathbb{R}^3$ |
| 无穷的来源 | 函数值在无穷多个点处变化 | 子域 $\mathcal{D}$ 的"切法"变化（域变量） |
| 底空间 | 固定（如 $[0,1]$） | 全域 $\mathbb{R}^3$ 固定 |
| 无穷维对应 | 函数空间 $\{f\}$ 的维数 | 子域族 $\{\mathcal{D}\}$ 的参数维数 |
| 叠加操作 | $\sum a_n e_n$（需收敛） | $\bigcup_{\mathcal{D}} \boxed{\mathcal{S}}_q(\mathcal{D})$（集合并置） |
...
| 拓扑关系 | 先合并？ | $\mathcal{Z}_q(\mathcal{D}_1)+\mathcal{Z}_q(\mathcal{D}_2)$ 与 $\mathcal{Z}_q(\mathcal{D}_1\cup\mathcal{D}_2)$ 关系 |
|:---|:---|:---|
| 全等重叠 | 是 | 不等（重复计算） |
| 内部相交 | 是 | 不等（相交区域重复） |
| 边界贴合 | 是 | 不等（共享边界重复） |
...
**证明**：  
有限支撑行变换仅作用于有限个域变量 $\{\xi_{j_1}, \dots, \xi_{j_k}\}$。由定义 6.1，每个域变量对应一个可逆重参量化方向。有限个方向的连续形变仍落在同一可逆邻域的有限交中。逆变换同样有限支撑，故为同胚。
...
**证明**：  
两子域各自具有公理 Ⅱ 互补分裂结构。相交条件保证交叉正负配对在雅可比化简后完全等价。交集商结构可唯一分解为两子域各自逆结构的直和，因此信息无丢失、粘合无矛盾。
...
- 当 $\mathcal{O}_1$ 与 $\mathcal{O}_2$ **严格分离**（$\mathcal{O}_1 \cap \mathcal{O}_2 = \varnothing$）时，无共享边界需消去：
  $$
  \mathcal{Z}_q(\mathcal{O}_1 \cup \mathcal{O}_2) = \mathcal{Z}_q(\mathcal{O}_1) + \mathcal{Z}_q(\mathcal{O}_2).
  $$

- 在其他三种拓扑关系（**全等重叠**、**内部相交**、**边界贴合**）下，存在共享区域或共享边界被重复计算：
  $$
  \mathcal{Z}_q(\mathcal{O}_1 \cup \mathcal{O}_2) \neq \mathcal{Z}_q(\mathcal{O}_1) + \mathcal{Z}_q(\mathcal{O}_2).
  $$

该性质不是 DCG 的缺陷，而是其**构造逻辑的一贯性**：积分是闭包的外在属性，不是构造的输入；构造完成后，边界唯一决定积分值。**分块积分之和仅在严格分离情形下等于并置闭包的积分**。
...
**证明**：  
对任意 $\mathcal{D}$，存在可逆重参量化 $\rho_{\mathcal{D}} : \mathcal{D} \to \mathcal{D}_0$ 与商映射 $\zeta_q$ 可交换。所有局部平凡化通过可逆邻域拓扑连通、无绕、无全局障碍，因此整体平凡。
...
$R_q^{(n)}$ 在基 $\{w_k\}$ 上的作用为
$$
R_q^{(n)} : w_k \mapsto w_{k+1 \bmod n},
$$
从而 $(R_q^{(n)})^n = \mathrm{id}$。

**性质 7.3（$r^k$ 的 120° 循环结构）**  
设 $n$ 为 3 的倍数，$r = e^{2\pi i / n}$，则对任意 $k$，$r^k, r^{k+1}, r^{k+2}$ 在复平面上相位差严格为 $120^\circ$。此性质是 $r^k$ 作为单位根的内禀特征，与 $d$ 的取值无关。它直接保证 $n$ 轴轮换在局部三元组层面上满足公理 Ⅱ 的互易性。

$w_k = r^k + k \cdot d$ 的三元组是在此 $120^\circ$ 骨架上叠加了线性平移项 $k \cdot d$ 的结果。平移项改变的是三元组在复平面上的绝对位置，不改变骨架的 $120^\circ$ 对称性。因此公理 Ⅱ 的判定只检验 $r^k$ 部分，平移部分由解陪域 $\mathfrak{R}_{12}$ 的 $\eta$ 参数独立调节。
...
**证明**：$\eta$ 的调整仅改变等差平移项 $k \cdot \eta$，不触及 $r^k$ 的 $120^\circ$ 循环骨架（后者是公理 Ⅱ 的保障来源），因此 $\mathrm{Ob}_{12}^{(\eta)} = 0$ 对所有 $\eta \in \mathfrak{R}_{12}$ 成立。
...
| 解陪域（Resolution Fiber Domain） | 吸收循环上同调粘合障碍的中间纤维层。其纤维由等差参数 $d$ 的调节空间构成，相位松弛参数 $\eta \in \mathfrak{R}_{12}$ 用于统一不同层的平移参数 |
| 等比等差构造 | $n$ 轴情形下 $w_k = r^k + k \cdot d$ 的统一代数结构 |
| 有限支撑扰动 | 行变换仅涉及有限个非零域变量列，保证拓扑同胚与良定义性 |
| 几何拓扑维数 | 承载闭包的空间流形的拓扑维数（$n \geq 3$） |
| 域变量维数 | 刻画子域"切法"的独立参数个数（有限或无穷） |
| 递归无穷 | 迭代深度无界导致的无穷 |
| 参数无穷 | 函数空间自由度导致的无穷 |
| 并置无穷 | 闭包数量趋于无穷导致的无穷 |
| 表示无穷 | 伽罗瓦表示层数量趋于无穷导致的无穷 |
...
初始闭包 $\mathcal{O}_0 := L(P_1,P_2)$，$P_1=(0,0),P_2=(1,0)$。生成映射将线段替换为四条折线路径，尖顶由 $R_{60^\circ}$ 构造。逆映射为抹平尖顶合并线段。三条边一致并置，极限闭包合法。周长发散，面积收敛至 $\frac{8}{5}\cdot\frac{\sqrt{3}}{4}$。

#### E.2　克莱因瓶

初始闭包 $\mathcal{O}_0 := [0,1]\times[0,1]$。边界识别：左右同向粘合，上下反向粘合。生成映射为 $2\times2$ 胞腔细分。逆映射为合并四个子胞腔并撤销标记。单次细分即完成，属有限闭包。$\mathcal{J}(\mathcal{O}_+)=\mathcal{J}(\mathcal{O}_-)$，通过公理 Ⅱ。

#### E.3　三维柱状提升

对任意合法二维闭包 $\mathcal{K}^{(2)}$，取 $\boxed{\mathcal{K}}^{(3)} = \mathcal{K}^{(2)} \times [0,1]$。三维互补分裂继承二维，复定积分张量分解：
$$
\mathcal{Z}_q(\boxed{\mathcal{K}}^{(3)}_\pm) = \mathcal{J}_q(\mathcal{K}^{(2)}_\pm) \otimes \Lambda([0,1]).
$$
二维互易性保证三维等式成立。完全合法（公理 Ⅰ–Ⅳ）。

#### E.4　常数斜率填充体

填充关联映射 $\Phi(x,y,z) = (x + k_x z,\ y + k_y z,\ z)$，$k_x,k_y$ 为常数。外延复定积分自动吸收平移因子 $\Xi_q$，互易性与 $k_x,k_y$ 无关。逆映射显式存在。完全合法（公理 Ⅰ–Ⅳ）。

#### E.5　变斜率填充体

填充斜率随深度变化，如 $k_x(z)=0.3\sin(2\pi z)$，$k_y(z)=0.2\cos(2\pi z)$。扰动衰减率 $\delta_n \sim n^{-\alpha}$：
- $\alpha > 1$：合法；
- $\alpha = 1$：启用公理 Ⅴ，个案判定；
- $\alpha < 1$：非法。
...
本附录旨在澄清 DCG 与标准数值积分/几何处理工具之间的适用边界，避免"DCG 能替代一切"或"DCG 不如传统工具"的误解。

#### F.1　核心对照表

| 场景 | 标准工具表现 | DCG 表现 | 结论 |
|:---|:---|:---|:---|
| **规则区域**（矩形、圆、光滑曲面） | `dblquad`、有限元精确 ✅ | 离散求和一致 ✅ | 辅助计算兼容 |
| **变边界填充**（边界函数良好） | 支持良好 ✅ | 逐片累加一致 ✅ | 数值可互校 |
| **分形边界**（科赫雪花、Sierpinski） | **无法直接处理** ❌ | 递归顶点序列原生支持 ✅ | DCG 优势领域 |
| **边界贴合**（两区域共享部分边界） | 需手动处理交界方向 | **公理 Ⅳ 保障合并后边界只出现一次** | DCG 显式处理，不依赖人工消重 |

#### F.2　边界贴合消重操作顺序

| 步骤 | 操作 | 说明 |
|:---|:---|:---|
| 1 | 构造 $\mathcal{D}_1$、$\mathcal{D}_2$（各自合法） | 各自通过公理 Ⅰ–Ⅲ |
| 2 | 验证 $\mathcal{D}_1 \cup \mathcal{D}_2$ 通过公理 Ⅳ | 并置后仍紧致，边界贴合已被识别 |
| 3 | 消除共享内面 | 合并后边界仅含外表面 |
| 4 | 在合并后外边界上计算 $\mathcal{Z}_q(\mathcal{D}_1 \cup \mathcal{D}_2)$ | **积分是最后一步** |

> **关键结论**：
> - 当两子域**严格分离**时，$\mathcal{Z}_q(\mathcal{D}_1 \cup \mathcal{D}_2) = \mathcal{Z}_q(\mathcal{D}_1) + \mathcal{Z}_q(\mathcal{D}_2)$。
> - 对于全等重叠、内部相交、边界贴合，必须合并后再积分，$\mathcal{Z}_q(\mathcal{D}_1) + \mathcal{Z}_q(\mathcal{D}_2)$ 不能代表并置闭包的积分。
> - 标准数值工具中"先分块积分再相加"的做法，在 DCG 中仅在严格分离时成立；否则必须合并消重。

#### F.3　验证实例（来自 §6.3 命题 6.1 的拓扑判定）

设 $\mathcal{D}_1, \mathcal{D}_3, \mathcal{D}_4, \mathcal{D}_5, \mathcal{D}_6$ 为合法子域，其拓扑关系判定如下：

| 子域对 | 拓扑关系 |
|:---|:---|
| $\mathcal{D}_1$ vs $\mathcal{D}_2$ | 全等重叠 |
| $\mathcal{D}_1$ vs $\mathcal{D}_3$ | 内部相交 |
| $\mathcal{D}_1$ vs $\mathcal{D}_4$ | **边界贴合** |
| $\mathcal{D}_1$ vs $\mathcal{D}_5$ | 严格分离 |
| $\mathcal{D}_1$ vs $\mathcal{D}_6$ | 内部相交 |
| $\mathcal{D}_3$ vs $\mathcal{D}_4$ | 内部相交 |
...
| $\mathcal{D}_1$ | 5 | 5 | 8 |
| $\mathcal{D}_4$ | 5 | 5 | 8 |
| $\mathcal{D}_1 \cup \mathcal{D}_4$（合并后） | 5 | 5 | 8 |

**关键验证**：
- $\mathcal{D}_1$ 的右侧面 $(x=2)$：是 $\mathcal{D}_1$ 的边界 ✅
- $\mathcal{D}_4$ 的左侧面 $(x=2)$：是 $\mathcal{D}_4$ 的边界 ✅
- 合并 $\mathcal{D}_1 \cup \mathcal{D}_4$ 后，$x=2$ 面：是内部面，不在边界上 ✅
- 合并后边界只包含外表面，共享内面被消除 ✅

**积分对比**：
| 子域 | $\mathcal{Z}_q$ 值 |
|:---|:---|
| $\mathcal{D}_1$ | -32.000000 |
| $\mathcal{D}_4$ | -48.000000 |
| $\mathcal{D}_1 + \mathcal{D}_4$（算术和） | -80.000000 |
| $\mathcal{D}_1 \cup \mathcal{D}_4$（并置闭包） | -64.000000 |

**结论表**：

| 关系 | 先合并？ | $\mathcal{Z}(\mathcal{O}_1)+\mathcal{Z}(\mathcal{O}_2)$ 等于 $\mathcal{Z}(\mathcal{O}_1\cup\mathcal{O}_2)$？ |
|:---|:---|:---|
| 全等重叠 | 是 | 否（-32 ≠ -64） |
| 内部相交 | 是 | 否（-72 ≠ -64） |
| 边界贴合 | 是 | 否（-80 ≠ -64） |
...
| 工具 | 边界贴合处理方式 | 结果 |
|:---|:---|:---|
| 标准数值工具 | 手动定向 + 分块积分相加 | 共享边界可能被重复计算，须人工干预 |
| DCG | 公理 Ⅳ 并置合法性审查 + 边界消重 | 共享边界只出现一次，结果由闭包外边界唯一决定 |

标准工具与 DCG 在规则区域上数值一致，在分形边界上 DCG 是唯一可行方案，在边界贴合情形下 DCG 的"消重"语义是显式且可检验的，不依赖手动定向。
...
| 符号 | 含义 |
|:---|:---|
| $\bigoplus$ | 可数代数累加（不依赖测度），适用于有限或无穷递归层 |
| $\text{F.P.}_{s=0}$ | 洛朗展开在 $s=0$ 处的常数项系数（有限部分） |
| $\mathcal{O}_+$、$\mathcal{O}_-$ | 闭包 $\mathcal{O}$ 在公理 Ⅱ 意义下的互补分裂两半 |
| $\boxed{\mathcal{S}}_q$ | 带有曲率参数 $q$ 的空间闭包 |
| $\mathfrak{D}$ | 全体合法子域的集合（空间域的底空间） |
...
| **DCG 3.0** | 新增第七部分「无穷维度的统一构造」，采用等比等差双模递推 $w_k = r^k + k \cdot d$ 给出任意 $n \geq 3$ 轴情形的完整代数构造；解陪域显示形式由等差参数 $d$ 自然给出；无穷维度从"展望"升格为"已完成构造"；修正 4.3 节 $R_q^3 = \mathrm{id}$ 的表述；修正 7.3 节性质 7.3，明确 120° 循环结构属于 $r^k$ 骨架而非 $w_k$。 |
| **DCG 3.1** | 在 1.1 声明中完善"操作顺序原则"，明确严格分离与其它三种情形的积分规则差异；在 3.3 节积分算子边界性中增加"并置闭包的积分操作流程"表格；在 6.5 节定理 6.1 后补充"拼接后积分的唯一性"精确表述；修正 4.3 节雅可比行列式表述，明确完整 $\mathbb{R}^3$ 上 $\det(J_{R_q}) = 1-q$ 而在商空间上保体积；附录 F 新增积分对比结论表，严格区分四种拓扑关系下的积分等式成立条件；全文修订编号及交叉引用。 |