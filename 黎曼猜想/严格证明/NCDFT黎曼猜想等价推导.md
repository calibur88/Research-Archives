...
**补充说明**：$\phi$ 的自变量须取为 $x = \log(n)/\log(N) \in [0,1]$。此时小素数（$n \ll \sqrt{N}$）获得 $\phi \approx 1$ 的全权重，大数区域按 $\cos^2$ 型光滑过渡到零。该定义与支撑集 $\operatorname{supp}\phi \subset [0,1]$ 严格一致。
...
**适用范围**：该 $L^2$ 误差估计在 $t \in [0, T(N)]$ 上有效，要求 $T(N) = o(\sqrt{N})$。因此，要检测前 $J$ 个零点，需满足 $\gamma_J < \sqrt{N}$，即 $J \log J \lesssim \sqrt{N}$。这表明 $N$ 需随检测零点数量平方增长，与第 6.3 节的构造性协议一致。
...
**补充说明**：$|D_N(t)|$ 在零点处取得局部极大值，对应于 $-\zeta'/\zeta$ 的简单极点。该结论与数值验证一致。
...
形成 Lorentzian 型峰。局部极大值点收敛到真实零点位置。

**补充说明**：$|D_N(t)|$ 在零点处取得局部极大值，对应于 $-\zeta'/\zeta$ 的极点发散。该结论与第 4.4 节的 Lorentz 共振公式及数值验证一致。
