...
7. [第六层：最终定理与全局光滑性猜想的证伪](#7-第六层最终定理)
...
\implies \text{高频能量 } \|\Delta_{\ge J} u\|_{L^2}^2 \text{ 指数衰减}
$$
...
实际速度场 $u = u_{\text{linear}} + w$，其中 $w$ 包含 $O(|x|^2)$ 余项、截断边界效应和 Leray 投影非局部项。

涡量方向 $\theta$ 的演化方程在核心区域为：
...
在 Riccati 爆破时间 $T^* \sim O(1)$ 内，$\|w\|_{H^1}$ 足够小，使得 $\theta(t)$ 不偏离 $0$ 超过 $\pi/4$（即 $\cos^2\theta \ge 1/2$）。

**步骤三：时间尺度比较**

- Riccati 爆破时间：$T^* \sim \frac{1}{C_2 2^{2j}} \ln(\cdots) = O(1)$（对于 $j=0, \nu=0.01$ 约为 $2.85$）
- 粘性扩散破坏局部化的时间尺度：$\tau_{\text{diff}} \sim \frac{2^{-2j}}{\nu} = 100$
- 非线性失稳时间尺度：$\tau_{\text{NL}} \sim \frac{1}{\lambda} = O(1)$

由于 $T^* \ll \tau_{\text{diff}}$，粘性扩散在爆破前不破坏局部化；由于线性双曲稳定性，非线性扰动在 $T^*$ 内可控。

**步骤四：Biot-Savart 局部同阶性**
...
7. **Chy 范数发散**：$\widetilde{R}_j \to \infty$ 蕴含 $R_j \to \infty$，由 A2 和定理 6.1 的局部同阶性，$|\Delta_j u| \sim 2^j R_j$，故

   $$
   \frac{2^{-j}|\Delta_j u|}{1+R_j^\gamma} \sim R_j^{1-\gamma} \to \infty,
   $$
...
4. **构造方式**：$u_0^{(\varepsilon)} = \lambda_\varepsilon u_0(\lambda_\varepsilon x)$，其中 $\lambda_\varepsilon = 2\varepsilon^{-1}$
...
对于满足以下附加条件的初值，全局光滑性成立：
- 初值能量足够小（Koch–Tataru 小数据理论）
- 初值具有特定对称性（轴对称无 swirl）
- 初值满足 Serrin 型可积条件

这些条件性结果不改变全局光滑性猜想的全称命题结构。猜想声称"**所有**光滑初值"，而我们构造了"**一个**爆破初值"——全称命题被单一反例彻底否定。
...
(3) \widetilde{R}_j → ∞  ⇒  R_j → ∞  
        |  
        v  
(4) A2 + 定理 6.1: |Δ_j u| ~ 2^j · R_j  (局部同阶性)  
        |  
        v  
...
| 8 | 定理 3.1 充分性（Chy 有限 ⇒ 光滑，情况划分） | [OK] |
| 9 | 定理 6.1 拉伸保持（核心区域旋转不变性） | [OK] |
| 10 | Riccati 方程精确解：dR/dt = C1 R^2 - C2 R | [OK] |
| 11 | Riccati 超阈值条件 R_0 &gt; C2/C1 导致有限时间爆破 | [OK] |
| 12 | 原始 Burgers 涡管 div u^{raw} = 0 | [OK] |
| 13 | 原始涡量 ω^{raw} = (0,0,2β) | [OK] |
| 14 | 原始拉伸项 ω^T S ω = 8αβ^2 | [OK] |
...
实际速度场 $u = u_{\text{lin}} + w$，其中 $w$ 包含 $O(|x|^2)$ 余项、截断边界效应和 Leray 投影非局部项。
