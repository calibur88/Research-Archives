...
局部几何 (探针 R_j)
        |
        v
Chy 范数 $C^{-1}_{\infty,\infty}$  $\xleftarrow{\text{局部-频率耦合}}$
        |
        v  (探针有界时等价)
Besov 范数 $B^{-1}_{\infty,\infty}$  $\xleftarrow{\text{纯频率全局}}$
        |
        v  (嵌入)
...
1. **局部 $L^2$ 涡量模**：$\widetilde{R}_j(t) = \sup_{x_0} \left[ \frac{1}{V_j} \int \phi_{j,x_0} |\omega|^2 \right]^{1/2}$
2. **Hölder 控制**：$R_j \leq \widetilde{R}_j$
3. **发散传递**：$\widetilde{R}_j \to \infty \implies R_j \to \infty$
4. **A2 + 定理 6.1（局部同阶性）**：$|\Delta_j u| \sim 2^j \cdot R_j$
5. **Chy 范数定义**：$m_j = \frac{2^{-j} |\Delta_j u|}{1 + R_j^\gamma}$
6. **爆破检测**：当 $R_j \to \infty$ 时，$m_j \sim c \cdot R_j^{1-\gamma} \to \infty$（因 $\gamma < 1$）
7. **Chy 范数发散**：$M(t) \to \infty$
8. **定理 3.1**：$M(t) \to \infty \iff T^* < \infty$
9. **结论**：解在有限时间爆破  
...
粘性扩散 $\Rightarrow$ 涡量分布区域扩大 $\Rightarrow$ 更大区域受应变率拉伸 $\Rightarrow$ 更强涡量 $\Rightarrow$ 更强扩散 $\Rightarrow \cdots \Rightarrow$ 失控  
