...
在 DCG 中，椭圆作为满足焦点约束与参数化光滑性的特殊闭包出现，是圆闭包向非常数焦距的自然推广，也是 $\mathcal{E}$-闭包分段协议的光滑极限。椭圆焦点 $F_1, F_2$ 的存在，等价于在底空间插入了两个**“环柄”（Handle）**。当观测者沿椭圆长轴绕行一周，曲率参数 $q$ 的局部投影（即半短轴与半长轴之比 $b/a$）产生非平凡单值性——这一现象恰好呼应了“CMB 四极八极对齐是宇宙大尺度绕元在局部坐标卡的投影”这一直觉。
...
| $\delta_n \sim 1/n$（**对数绕元临界状态**） | $\Phi(s)=\zeta(s+1)$，$s=0$ 简单极点 | Dixmier 迹 / 梅林有限部分 | 🔄 公理 Ⅴ 启用 |
| $\delta_n \sim n^{-\alpha}$，$\alpha<1$ | $\Phi(s)$ 在 $s=0$ 非可去奇性 | Dixmier 迹失效 | ❌ 非法 |

**判定树**：$\alpha>1$ → 合法；$\alpha=1$（对数绕元临界） → 个案（公理 Ⅴ）；$\alpha<1$ → 非法。
...
| 四 | **表示无穷** | 伽罗瓦表示层数量 $\to \infty$ | 纤维丛结构 | 层上同调 + 解陪域筛选 |
...
空间域的整体丛结构由表示层筛选器决定，其底空间 $\mathfrak{D}$ 上可能存在非平凡 **绕元（$[\gamma]\in\pi_1$）**，这些绕元阻碍全局截面的一致定义。整体丛结构表述为：

- 若所有障碍类 $\mathrm{Ob}_{12}=0$，则空间域退化为**平凡纤维丛**：
  $$
  \boxed{\mathcal{S}}_q^{\infty} \cong \mathfrak{D} \times \mathbb{R}(\omega_q).
  $$

- 若存在非零障碍类（即非平凡绕元），则空间域为**非平凡纤维丛**。此时不存在整体定义的全局截面；必须在每个局部坐标卡 $U_\alpha$（即“局部补丁”）上分别定义平凡化，再通过**解陪域 $\mathfrak{R}_{12}$** 吸收粘合障碍，使得局部截面在各补丁交界处相容。换言之，底空间 $\mathfrak{D}$ 上的非平凡 $\pi_1$ 阻碍了全局平凡化，局部坐标卡 $U_\alpha$ 如同“局部补丁”，在其中看得见各向同性，却看不见全局绕行。

*证明概要*：对任意 $\mathcal{D}$，存在可逆重参量化 $\rho_{\mathcal{D}} : \mathcal{D} \to \mathcal{D}_0$ 与商映射 $\zeta_q$ 可交换。局部平凡化通过可逆邻域拓扑连通，但若 $\pi_1(\mathfrak{D}) \neq 0$，绕非平凡绕元一周后纤维自同构非恒等，故整体非平凡。
...
**元定理（解陪域完备性）**：  
对于 DCG 中任意非零层上同调障碍 $\mathrm{Ob}_{12}$，若存在一个紧致、可逆、互易、一致的解陪域 $\mathfrak{R}_{12}$ 使得 $\mathrm{Ob}_{12}^{(\eta)} = 0$，则该障碍不构成合法性失效，而仅构成 **"构造复杂度增加"** 的标志。
...
**定义 6.5（层上同调粘合障碍）**  
设 $[\rho_1], [\rho_2]$ 为两个表示层，其纤维在交集 $\mathcal{D} \in \mathfrak{D}_{[\rho_1]} \cap \mathfrak{D}_{[\rho_2]}$ 上的过渡函数为 $g_{12}$。定义层上同调障碍类：
$$
\mathrm{Ob}_{12} := [g_{12}] \in \check{H}^1(\mathcal{D}, \mathcal{G}),
$$
其中 $\mathcal{G}$ 为规范变换层。

> **注**：非零 $\mathrm{Ob}_{12}$ 表征了非平凡绕元的存在，其单值性（Monodromy）由 $\pi_1$ 的表示给出。

- 若 $\mathrm{Ob}_{12} = 0$：两层可无碍粘合；
- 若 $\mathrm{Ob}_{12} \neq 0$：需引入解陪域。

**定义 6.6（解陪域）**  
设 $[\rho_1], [\rho_2] \in \operatorname{Rep}_2(G)$，其过渡函数 $g_{12}$ 的层上同调障碍为 $\mathrm{Ob}_{12} \neq 0$。称纤维层 $\mathfrak{R}_{12}$ 为 $g_{12}$ 的**解陪域**，若存在相位松弛参数 $\eta \in \mathfrak{R}_{12}$ 使得提升过渡函数
$$
...
**命题 6.4（解陪域存在性）**  
若 $g_{12}$ 的障碍类 $[\mathrm{Ob}_{12}] \in \check{H}^1$ 为**挠元（torsion）**，则存在**有限维解陪域** $\mathfrak{R}_{12}$。  
...
两者共享同一套表示层筛选器（伽罗瓦群 + 层上同调 + 解陪域），但作用于不同的层次。
...
**性质 7.3（$n$-绕元生成骨架）**  
设 $r = e^{2\pi i / n}$，则 $\{r^k\}_{k=0}^{n-1}$ 在复平面上构成一个 **$n$-绕元生成骨架**：$r^{k+1}$ 由 $r^k$ 旋转 $2\pi/n$ 得到，$(R_q^{(n)})^n = \mathrm{id}$。这一循环结构是 $w_k$ 的普遍代数基础，直接保证 $n$ 轴轮换在任意 $n \geq 3$ 时满足公理 Ⅱ 的互易性——互补配对由旋转对称性自然诱导，与等差平移项 $k \cdot d$ 无关。

在该骨架中，$r^k$ 部分生成的是 **“纯旋转绕元”**（对应保向旋转骨架），而 $k\cdot d$ 平移项是 **“等差滑移绕元”**（对应非挠伸展），两者清晰区分了挠与非挠障碍的起源。

当 $n$ 恰好为 $3$ 的倍数时，上述 $n$ 重骨架在局部三元组 $\{r^k, r^{k+1}, r^{k+2}\}$ 上额外呈现 $120^\circ$ 相位差。此 $120^\circ$ 结构是 $n=3m$ 时的内禀特例，并非公理 Ⅱ 的普遍必要条件。无论 $n$ 是否为 $3$ 的倍数，$n$ 重循环骨架本身已足以支撑互易性判定。

$w_k = r^k + k \cdot d$ 的任意相邻三元组是在此 $n$-绕元生成骨架上叠加了线性平移项 $k \cdot d$ 的结果。平移项改变的是三元组在复平面上的绝对位置，不改变骨架的循环对称性。因此公理 Ⅱ 的判定只检验 $r^k$ 部分，平移部分由解陪域 $\mathfrak{R}_{12}$ 的 $\eta$ 参数独立调节。
...
*证明概要*：$\eta$ 的调整仅改变等差平移项 $k \cdot \eta$，不触及 $r^k$ 的 $n$-绕元生成骨架（后者是公理 Ⅱ 的保障来源），因此 $\mathrm{Ob}_{12}^{(\eta)} = 0$ 对所有 $\eta \in \mathfrak{R}_{12}$ 成立。
...
| 解陪域（Resolution Fiber Domain） | 吸收层上同调粘合障碍的中间纤维层，由等差参数 $d$ 的调节空间构成 |
| 等比等差构造 | $n$ 轴情形下 $w_k = r^k + k \cdot d$ 的统一代数结构 |
| 有限支撑扰动 | 行变换仅涉及有限个非零域变量列，保证拓扑同胚与良定义性 |
| **和乐（Holonomy）** | 在 DCG 中，指向量或纤维沿闭包边界平行移动一周后，在纤维上产生的**偏差变换**。若底空间（模空间/空间域）的联络平坦（曲率为零），则和乐退化为单值性。 |
| **单值性（Monodromy）** | 在平坦（无曲率）闭包或参数簇中，绕非平凡**绕元**（$[\gamma]\in\pi_1$）走一圈后，局部截面（切向量/多极矩）无法回到自身的**全局投影效应**。在 DCG 中由障碍类 $\mathrm{Ob}_{12}$ 的非零性表征。 |
...
| DCG 2.5 | — | 新增第六部分「空间域整体变换」；无穷谱系扩展为四层级；公理 Ⅴ 扩展判定算法（含解陪域恢复）；伽罗瓦表示层筛选器；层上同调粘合障碍。 |
| DCG 3.0 | — | 新增第七部分「无穷维度的统一构造」，采用等比等差双模递推 $w_k = r^k + k \cdot d$ 给出任意 $n \geq 3$ 轴情形的完整代数构造；解陪域显式形式由等差参数 $d$ 自然给出；无穷维度从"展望"升格为"已完成构造"；修正 4.3 节 $R_q^3 = \mathrm{id}$ 的表述；修正 7.3 节性质 7.3。 |
| DCG 3.1 | — | 在 1.1 声明中完善"操作顺序原则"，明确严格分离与其它三种情形的积分规则差异；在 3.3 节积分算子边界性中增加"并置闭包的积分操作流程"表格；在 6.5 节定理 6.1 后补充"拼接后积分的唯一性"精确表述；修正 4.3 节雅可比行列式表述；附录 F 新增积分对比结论表；全文修订编号及交叉引用。进一步修正 7.3 节性质 7.3，将 120° 局部对称明确降级为 $n=3m$ 时的内禀特例；新增 §3.2.5 平滑椭圆闭包。 |
| DCG 3.2 | — | 新增可选公理 Ⅵ（唯一性）与可选公理 Ⅶ（复合性），二者均为可选约束，声明且核验失败者仅撤销对应标签；声明组合与检验范围明确为四种状态；实例更新涵盖柱状提升、混合曲率、科赫雪花并置等场景。 |
| DCG 3.3 | — | 新增附录 E.6「极度尖锐科赫雪花变体（二维）」边界案例分析及附录 E.7「三维分层复合——合法性救赎」；展示了公理 Ⅴ 的判别力、公理 Ⅲ 的“参数可逆”与“构造可逆”之区分，以及公理 Ⅶ 在三维分层复合中的介入逻辑。 |
| **DCG 3.4** | — | **全面升级核心术语以匹配几何直觉**：将上同调类型从 `HC^1` 明确为 Čech 层上同调 $\check{H}^1(\mathcal{D}, \mathcal{G})$；空间域平凡丛表述修正为非平凡纤维丛结构，明确指出非平凡绕元阻碍全局平凡化；7.3 节 $n$ 重循环对称正式命名为 **“$n$-绕元生成骨架”**，并区分“纯旋转绕元”与“等差滑移绕元”；3.2.5 椭圆闭包明确焦点为**“环柄”**并指出其沿长轴产生非平凡单值性；5.4 节引入 **“对数绕元临界状态”** 术语；附录 D 新增“和乐”与“单值性”的 DCG 精确定义。 |